...
public class GameManager : MonoBehaviourPun
{
    ...
    private GameObject VRSpawnPos; // VR 위치 저장할 변수
    ...
    private void Start()
    {
        ...
        if (userId == 1)//vr?????? ????
        {
            ...
            // ???????????? ???? ?????????????? ???? ????  
            // ???? ???? ?????????? ???????? ???? ???????? ???? ?????? ???????????? ????
            PhotonNetwork.Instantiate(VRPlayerPrefab.name, GameObject.Find("VRSpawnPos").transform.position, Quaternion.identity);
            //playerCnt++;
        }
        else
        {
